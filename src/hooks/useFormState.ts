 ...
 *
 * @example
 * const [formState, { onInputChange }, , refState] = useFormState<SeatFormState>(EMPTY_STATE, initialState)
 *
 ...
 * const [formErrors, { checkFieldErrorsOnFormStateChange }] = useFormErrors<SeatFormState>(validationRules)
 *
 * const [formState, { onInputChange, onDropDownChange, onToggle }, { resetForm }] = useFormState<SeatFormState>(
 *   emptySeatForm,
 *   initialFormState,
...
export function useFormState<S extends AnyObject = AnyObject>(
  emptyState: S = {} as S,
  initialState: Partial<S> = emptyState,
